...
local function data_diff()
    ...
    if in_diff/1024 > 1024 then
        obj.kbin = string.format("%6.2f %5s", in_diff/1024/1024, ' mb/s')
    else
        obj.kbin = string.format("%6.2f %5s", in_diff/1024, ' kb/s')
    end
    if out_diff/1024 > 1024 then
        obj.kbout = string.format("%6.2f %5s", out_diff/1024/1024, 'mb/s')
    else
        obj.kbout = string.format("%6.2f %5s", out_diff/1024, 'kb/s')
    end
    -- local disp_str = '⥄ ' .. obj.kbout .. '\n⥂ ' .. obj.kbin
    -- local disp_str = obj.kbout .. obj.kbin
    local disp_str = string.format("%-15s %-15s", '⬆' .. obj.kbout, '⬇' .. obj.kbin)
